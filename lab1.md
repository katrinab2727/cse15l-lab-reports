...
[user@sahara ~]$
```
The working directory was `/home/lecture1` when `cd` was run. Using `cd` without an argument sets the working directory to `home`, so the current directory was changed from `lecture1` to `home`.
