...
`/home/linux/ieng6/oce/95/kbosler/.ssh` is the path that contains `authorized_keys`(where the public key is located ?), `id_rsa`, and `id_rsa.pub` (public key) that allows me to log into my `ieng6` account without being asked for a password.
